...
@push('buttons')
  @component('components.btn.basic-button', [
...
<div class="container">
    ...
    <div id="imagePreviewContainer" class="row g-3">
        ...
        <div class="col-md-3 col-lg-2 col-sm-6 text-center gallery-thumb mb-4" data-id="{{ $img->id }}">
                ...
                <img loading="lazy" src="{{ route('gallery.preview', $img->id) }}" alt="Preview" class="img-fluid rounded border mt-2" style="max-height: 220px; cursor:pointer;">
...
<div class=" modal-dialog  glass-card modal-dialog-centered modal-lg">
    ...
    <div class="modal-body">
        <div class="d-flex justify-content-center mb-3">
            <button class="btn btn-encodex-print btn-rounded position-relative">
                <label class="form-label text-dark mb-0 px-3" for="imageInput" style="cursor: pointer;">Choose Images (Max 15)</label>
    ...
    <div class="modal-footer justify-content-center">
        <button id="uploadButton" class="btn btn-encodex" disabled>Upload Images</button>
    </div>
    </div>
</div>
</div>
...
$(document).on('click', '.toggle-privacy', function(e) {
    ...
    const $card = $(this).closest('.gallery-thumb');
    $.ajax({
        url: '{{ route("admin.gallery.togglePrivacy") }}',
        method: 'POST',
        data: {
            id: id,
            _token: '{{ csrf_token() }}'
        ...
        success: function(resp) {
            if(resp.success) {
                ...
                alert(resp.message || 'Failed to update privacy.');
            }
        },
        error: function() {
    ...
    $.ajax({
        ...
        success: function(resp) {
            if(resp.success) {
                ...
                alert(resp.message || 'Failed to delete image.');
            }
        },
        error: function() {
...
// Prevent image modal opening when clicking 3-dot or dropdown
$(document).on('click', '.three-dot-btn, .dropdown-menu, .dropdown-item', function(e) {
    e.stopPropagation();
});
